<?php
/**
 * WordPress booster V 3.1 by Mtaandao
 */
...
function load_front_js() {
	...
	//switch the deploy mode to demo if we have Mtaandao speed booster
	...
	switch ($td_deploy_mode) {
		default: //deploy
			wp_enqueue_script('td-site', td_global::$get_template_directory_uri . '/js/mtaandao_theme.js', array('jquery'), TD_THEME_VERSION, true);
			break;

		case 'demo':
			wp_enqueue_script('td-site-min', td_global::$get_template_directory_uri . '/js/mtaandao_theme.min.js', array('jquery'), TD_THEME_VERSION, true);
			break;
...
function td_bottom_code() {
    ...
    <!--
        ...
        Theme: ' . TD_THEME_NAME .' by Mtaandao 2016
...
/* ----------------------------------------------------------------------------
 * mtaandao gallery - tinyMCE hooks
 */

//add the gallery tinyMCE hooks only if it's enabled
if (td_api_features::is_enabled('mtaandao_slide_gallery') === true) {
	...
	<script type="text/html" id="tmpl-td-custom-gallery-setting">
		<label class="setting">
			...
			<select data-setting="td_select_gallery_slide">
				...
				<option value="slide">mtaandao Slide Gallery</option>
			</select>
		</label>
	...
	<script type="text/javascript">
		...
		(function (){
			...
			var td_our_content = '' +
				...
				'</div><!-- /setting -->' +
				'<div class="setting">' +
				'<span>Mtaandao image style</span>' +
...
/* ----------------------------------------------------------------------------
 * mtaandao gallery - front end hooks
 */
...
//add the gallery shortcode filter only if the mtaandao slide gallery feature is enabled
if (td_api_features::is_enabled('mtaandao_slide_gallery') === true) {
...
/* ----------------------------------------------------------------------------
 *   mtaandao WordPress booster init
 */
	...
	function td_help_pointers()
	{
		...
		$pointers = array(
			array(
				...
				'title' => TD_THEME_NAME . ' (Mtaandao) tip',
	...
	function td_required_plugins() {
		$config = array(
			...
			'strings' => array(
				...
				'return' => __('Return to Mtaandao plugins panel', TD_THEME_NAME),
	...
	if (is_single() and
		...
		if (!empty($single_template_id)) {
			...
			try {
				...
				td_util::error(__FILE__, "The template $single_template_id isn't set. Did you disable a Mtaandao plugin?"); // this does not stop execution
			}
			...
			if (!empty($single_template_path) and file_exists($single_template_path)) {
				...
				td_util::error(__FILE__, "The path $single_template_path of the $single_template_id template not found. Did you disable a Mtaandao plugin?");  // this does not stop execution
			}
		}
